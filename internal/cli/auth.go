...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Fiken",
...
func resolveOAuthCredentials(cmd *cobra.Command, flags *rootFlags, cfg *config.Config, clientID, clientSecret string) (string, string, error) {
	...
	if flags.noInput {
		return "", "", fmt.Errorf("OAuth2 client ID is required; pass --client-id, set FIKEN_CLIENT_ID, or run 'fiken-cli login' without --no-input")
	}
...
func printOAuthCredentialHint(w io.Writer) {
	fmt.Fprintln(w, "Create OAuth credentials at: https://fiken.no/")
}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				fmt.Fprintf(w, "  %s Not authenticated. Run 'fiken-cli login' to authenticate.\n", red("FAIL"))
				return nil
			}
			...
			if cfg.TokenExpiry.IsZero() {
				...
				if cfg.RefreshToken != "" {
					...
					fmt.Fprintf(w, "  %s Token expired. Run 'fiken-cli login' to re-authenticate.\n", red("FAIL"))
				}
			}
