...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fiken-cli doctor
  fiken-cli doctor --json
  fiken-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "fiken-cli login"
					report["auth_key_url"] = "https://fiken.no/"
				} else {
					authConfigured = true
			...
			if strings.Contains("Personal API token (Fiken -> Settings -> API -> Personlige API-nokler). Sent as a Bearer token; never expires; ToS-clean for your own companies. Set this OR run 'auth login' for OAuth2.", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "FIKEN_API_TOKEN")
				if os.Getenv("FIKEN_API_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "FIKEN_API_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("FIKEN_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "FIKEN_API_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "FIKEN_API_TOKEN optional")
			}
			if os.Getenv("FIKEN_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "FIKEN_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "FIKEN_CLIENT_ID set during auth login")
			}
			if os.Getenv("FIKEN_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "FIKEN_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "FIKEN_CLIENT_SECRET set during auth login")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "fiken-cli"
						verifyPath := "/user"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'fiken-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'fiken-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'fiken-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'fiken-cli sync' to refresh."
	}
	return report
}
