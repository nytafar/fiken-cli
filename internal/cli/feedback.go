...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "fiken-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FIKEN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FIKEN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fiken-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/fiken-cli/feedback.jsonl.
When ` + "`FIKEN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FIKEN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fiken-cli feedback list
  fiken-cli feedback list --limit 5
  fiken-cli feedback list --json`,
