...
In local mode: searches locally synced data only.`,
  ...
  fiken-cli search "error timeout"

  # Force local search only
  fiken-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  fiken-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  fiken-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'fiken-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
