...
type Config struct {
	...
	FikenApiToken string          `toml:"api_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("FIKEN_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "fiken-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("FIKEN_API_TOKEN"); v != "" {
		cfg.FikenApiToken = v
		cfg.markEnvOverride("FikenApiToken")
		cfg.AuthSource = "env:FIKEN_API_TOKEN"
	}
	if v := os.Getenv("FIKEN_CLIENT_ID"); v != "" {
		cfg.ClientID = v
		cfg.markEnvOverride("ClientID")
		cfg.AuthSource = "env:FIKEN_CLIENT_ID"
	}
	if v := os.Getenv("FIKEN_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.markEnvOverride("ClientSecret")
		cfg.AuthSource = "env:FIKEN_CLIENT_SECRET"
	}
	...
	if cfg.AuthSource == "" && cfg.FikenApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("FIKEN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("FIKEN_AUTHORIZATION_URL"); v != "" {
		cfg.AuthorizationURL = v
	}
	if v := os.Getenv("FIKEN_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.FikenApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:FIKEN_API_TOKEN"
		}
		return "Bearer " + c.FikenApiToken
	}
	return ""
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.FikenApiToken = ""
	delete(c.envOverrides, "FikenApiToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["FikenApiToken"] {
			out.FikenApiToken = c.fileConfig.FikenApiToken
		}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "FikenApiToken":
		c.fileConfig.FikenApiToken = c.FikenApiToken
	}
}
